...
import main.java.edu.kaist.cs.globals.ObjectTuple;
import main.java.edu.kaist.cs.globals.PositionTuple;
import main.java.edu.kaist.cs.globals.Triple;
import main.java.edu.kaist.cs.tools.ETRIExecutor;
import main.java.edu.kaist.cs.weka.classifiers.ArgumentWekaWrapper;
...
public class SRDF {
	...
	public ArrayList<String> extractRelations(String extractedPOS) {
		...
		try {
			...
			for (int i = 0; i < allOfType.size(); i++) {
				...
				trainingData = "'" + lemma + "','" + type + "'," + postposition
						+ "," + relation;
		...
		try {
			...
			for (int i = 0; i < morpLength; i++) {
				...
				if (tempType.equals("JKS") || tempType.equals("JKC")
					...
					else if (!tempLemma.equals("의") && !tempLemma.equals("와")
							&& !tempLemma.equals("과")) {
	...
	public void writeRelationTestingFile(ArrayList<String> relationFeatures) {
		try {
			...
			filebw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("data/testing-relation.arff")));
	...
	public void writeArgumentTestingFile(ArrayList<String> argumentFeatures) {
		try {
			filebw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("data/testing-arguments.arff")));
	...
	public void relationLearning() {
		try {
			...
			loaderTraining.setFile(new File("data/training-relation.arff"));
			relationTraining = loaderTraining.getDataSet();
			relationTraining
					.setClassIndex(relationTraining.numAttributes() - 1);
			...
			relationTrainingSet = Filter.useFilter(relationTrainingSet,
					relationFilter);
	...
	public void argumentLearning() {
		try {
			...
			loaderTraining.setFile(new File("data/training-arguments.arff"));
			argumentTraining = loaderTraining.getDataSet();
			argumentTraining
					.setClassIndex(argumentTraining.numAttributes() - 1);
			...
			argumentTrainingSet = Filter.useFilter(argumentTrainingSet,
					argumentFilter);
	...
	public ArrayList<String> relationClassify() {
		...
		try {
			...
			for (int i = 0; i < dataTestingSet.numInstances(); i++) {
				...
				if (relationClassifier.classifyInstance(dataTestingSet
						.instance(i)) == 0) {
	...
	public void argumentClassify() {
		...
		classifiedArguments1.clear();
		classifiedArguments2.clear();
		...
		try {
			...
			loaderTesting.setFile(new File("data/testing-arguments.arff"));
			Instances dataTesting = loaderTesting.getDataSet();
			dataTesting.setClassIndex(dataTesting.numAttributes() - 1);
			...
			for (int i = 0; i < dataTestingSet.numInstances(); i++) {
				...
				if (classifier.classifyInstance(dataTestingSet.instance(i)) == 0) {
					...
					inst = dataTesting.instance(i);
					lemmaOfObject.add(new ObjectTuple(inst.stringValue(0), inst
							.stringValue(5)));
					positionOfObject.add(String.valueOf(inst.value(4)));

					// System.out.println("object: " + inst.stringValue(0) +
					// " + "
					// + inst.stringValue(5));
	...
	public ArrayList<Triple> makeSRDF() {
		...
		for (int i = 0; i < lemmaOfSubject.size(); i++) {
			sbjStack.add(new PositionTuple(lemmaOfSubject.get(i),
					positionOfSubject.get(i)));
		}

		for (int i = 0; i < lemmaOfRelation.size(); i++) {
			relStack.add(new PositionTuple(lemmaOfRelation.get(i),
					positionOfRelation.get(i)));
		}

		for (int i = 0; i < lemmaOfObject.size(); i++) {
			objStack.add(new PositionTuple(lemmaOfObject.get(i),
					positionOfObject.get(i)));
		}
		...
		if (sbjStack.size() != 0) {
			...
			for (int i = sbjStack.size() - 1; i >= 0; i--) {
				...
				for (int j = relStack.size() - 1; j >= 0; j--) {
					...
					String originRelLemma = relation.getLemma();
					String relLemma = relation.getLemma() + "#" + time;
					int relPosition = (int) Double.parseDouble(relation
							.getPosition());
					...
					if (j > 0) {
						...
						preRelPosition = (int) Double.parseDouble(preRelation
								.getPosition());
					} else {
						preRelPosition = 0;
					}
					...
					for (int k = objStack.size() - 1; k >= 0; k--) {
						...
						int objPosition = (int) Double.parseDouble(object
								.getPosition());

						String objPost = objTuple.getPostposition();
						String objLemma = objTuple.getObject();

						if (relPosition > objPosition
								&& preRelPosition < objPosition) {

							if (originRelLemma.equals("이")) {
								if (!vcpReification) {
									t = new Triple(sbjLemma, relLemma,
											objLemma, objPosition + relPosition);
									srdf.add(t);

									vcpReification = true;
								} else {
									t = new Triple(relLemma, objPost, objLemma,
											objPosition + relPosition);
								...
								if (objReification) {
									if (objPost.equals("을")
											|| objPost.equals("를")) {
										t = new Triple(postLemma, relLemma,
												objLemma, objPosition
														+ relPosition);
										srdf.add(t);
										reification = true;
									} else {
										if (!reification) {
											t = new Triple(postLemma, relLemma,
													"ANONYMOUS", objPosition
															+ relPosition);
											srdf.add(t);
											reification = true;
										}
										t = new Triple(relLemma, objPost,
												objLemma, objPosition
														+ relPosition);
										srdf.add(t);
									}
							...
							if (!relReification) {
								t = new Triple(sbjLemma, relLemma, "ANONYMOUS",
										objPosition + relPosition);
								srdf.add(t);
								objReification = true;
								relReification = true;
	...
	static class TripleDescCompare implements Comparator<Triple> {
		...
		public int compare(Triple arg0, Triple arg1) {
			...
			return arg0.getPosition() > arg1.getPosition() ? -1 : arg0
					.getPosition() < arg1.getPosition() ? 1 : 0;
		}
